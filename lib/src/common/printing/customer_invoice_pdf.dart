...
Future<Document> getCustomerInvoicePdf(BuildContext context, WidgetRef ref,
  ...
  final customerData =
      customerDbCache.getItemByDbRef(transactionData['nameDbRef']);
  final salesmanDbCache = ref.read(salesmanDbCacheProvider.notifier);
  final salesmanData =
      salesmanDbCache.getItemByDbRef(transactionData['salesmanDbRef']);
  final type =
      translateDbTextToScreenText(context, transactionData['transactionType']);
  ...
  final paymentType =
      translateDbTextToScreenText(context, transactionData['paymentType']);
  final date = formatDate(transactionData['date']);
  final subtotalAmount =
      doubleToStringWithComma(transactionData['subTotalAmount']);
  final discount = doubleToStringWithComma(transactionData['discount']);
  final currency =
      translateDbTextToScreenText(context, transactionData['currency']);
  ...
  final totalNumOfItems =
      doubleToStringWithComma(_calculateTotalNumOfItems(items));
  ...
  final debtBefore = doubleToStringWithComma(
      customerScreenData['totalDebt'] - transactionData['totalAmount']);
...
pw.Widget _invoicePage(
  ...
  return pw.Column(
    ...
    children: [
      ...
      _buildFirstRow(context, arabicFont, customerName, customerPhone,
          customerRegion, paymentType),
      ...
      _buildItems(
          arabicFont,
          items.sublist(
              startItem, items.length < endItem ? items.length : endItem),
          startingSequence: startSequence),
      pw.SizedBox(height: 8),
      if (addTotals)
        _totals(arabicFont, subtotalAmount, discount, debtBefore, debtAfter,
            currency, notes, totalNumOfItems, itemsWeigt),
      pw.Spacer(),
      _signituresRow(arabicFont),
      pw.SizedBox(height: 5),
      footerBar(
          arabicFont,
          'الشركة غير مسؤولة عن انتهاء الصلاحية بعد استلام البضاعة',
...
num _calculateTotalNumOfItems(List<dynamic> items) {
  ...
  for (int i = 0; i < items.length; i++) {
    numItems +=
        items[i]['soldQuantity'].toInt() + items[i]['giftQuantity'].toInt();
  }
  return numItems;
}

pw.Widget _buildFirstRow(
    BuildContext context,
    Font arabicFont,
    String customerName,
    String customerPhone,
...
pw.Widget _itemTitles(Font arabicFont) {
  final childWidget = pw.Row(
    mainAxisAlignment: pw.MainAxisAlignment.spaceAround,
    children: [
      arabicText(arabicFont, 'المجموع',
          width: 70, isTitle: true, textColor: PdfColors.white),
      arabicText(arabicFont, 'السعر',
          width: 70, isTitle: true, textColor: PdfColors.white),
      arabicText(arabicFont, 'الهدية',
          width: 70, isTitle: true, textColor: PdfColors.red),
      arabicText(arabicFont, 'العدد',
          width: 70, isTitle: true, textColor: PdfColors.white),
      arabicText(arabicFont, 'اسم المادة',
          width: 200, isTitle: true, textColor: PdfColors.white),
      arabicText(arabicFont, 'ت',
          width: 40, isTitle: true, textColor: PdfColors.white),
    ],
  );
...
pw.Widget _itemsRow(Font arabicFont, String sequence, String name,
    String quantity, String gift, String price, String total) {
  return pw.Container(
    height: 20,
    width: 554,
    ...
    child: pw.Row(
      ...
      children: [
        ...
        arabicText(arabicFont, quantity,
            width: 70, isBordered: true, fontSize: 9),
        arabicText(arabicFont, name, width: 200, isBordered: true, fontSize: 9),
        arabicText(arabicFont, sequence,
            width: 40, isBordered: true, fontSize: 9),
      ],
    ),
  );
}
  ...
  return pw.Container(
    ...
    child:
      ...
      _invoiceAmountColumn(
          arabicFont, totalAmount, discount, debtBefore, debtAfter, currency),
...
pw.Widget _weightColumn(
    Font arabicFont, String notes, String itemsNumber, String itemsWeigt) {
  return pw.Column(
    mainAxisAlignment: pw.MainAxisAlignment.start,
    children: [
...
pw.Widget _invoiceAmountColumn(Font arabicFont, String totalAmount,
    String discount, String debtBefore, String debtAfter, String currency) {
  return pw.Column(
    mainAxisAlignment: pw.MainAxisAlignment.spaceBetween,
    children: [
      _totalsItem(arabicFont, 'مبلغ القائمة', totalAmount, darkBgColor,
          textColor: PdfColors.white),
      _totalsItem(arabicFont, 'الخصم', discount, lightBgColor),
      _totalsItem(arabicFont, 'الدين السابق', debtBefore, lightBgColor),
      _totalsItem(arabicFont, 'الدين الحالي', debtAfter, darkBgColor,
          textColor: PdfColors.white),
...
pw.Widget _totalsItem(
    Font arabicFont, String text1, String text2, PdfColor bgColor,
    {double width = 200, PdfColor textColor = PdfColors.black}) {
  return pw.Container(
    decoration: pw.BoxDecoration(
      borderRadius:
          const pw.BorderRadius.all(Radius.circular(4)), // Rounded corners
    ...
    child: pw.Row(
      ...
      children: [
        arabicText(arabicFont, text2, width: 80, textColor: textColor),
        pw.Spacer(),
